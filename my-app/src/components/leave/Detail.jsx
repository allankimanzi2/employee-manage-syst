...
const Detail = () => {
    ...
    useEffect(() => {
      const fetchLeave = async () => {
        try {
          const response = await axios.get(`http://localhost:5000/leave/detail/${id}`, {
            headers: {
    ...
    const changeStatus = async (id, status) =>
        try {
            const response = await axios.put(`http://localhost:5000/leave//${id}`, {status}, 
                 {
              headers: {
                Authorization: `Bearer ${localStorage.getItem('token')}`,
              },
    ...
    if (!leave ?|| !employee.userId || !employee.department) {
    ...
    {leave ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="flex space-x-3 mb-2">
              ...
              <p className="font-medium">{new Date(leave.status)}</p>
        </div>
      </div>
    );
  };
