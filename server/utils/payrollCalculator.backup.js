...
const addSalary = async (req, res) => {
  try {
    ...
    // --------------------------------------------------------
    // NORMALIZE LUNCH ALLOWANCE
    // --------------------------------------------------------

    const qualifyingOnSiteDays =
      toNonNegativeNumber(onSiteDays);

    const configuredLunchRate =
      lunchRate == null
        ? 100
        : toNonNegativeNumber(lunchRate);

    const explicitLunchAllowance =
      lunchAllowance != null
        ? toNonNegativeNumber(lunchAllowance)
        : undefined;

    // --------------------------------------------------------
    // NORMALIZE OTHER EARNINGS
    // --------------------------------------------------------
    ...
    const payroll = calculatePayroll({
      ...
      onSiteDays:
        qualifyingOnSiteDays,

      lunchRate:
        configuredLunchRate,

      lunchAllowance:
        explicitLunchAllowance,
    ...
    const {
      ...
      lunchAllowance: calculatedLunchAllowance,
      ...
      loanDeductions: calculatedLoanDeductions,
      advanceDeductions:
      ...
      salaries.reduce(
        (acc, salary) => {
          ...
          const lunchAllowance =
            Number(salary.lunchAllowance) || 0;

          const tripEarnings =
            Number(salary.tripEarnings) || 0;
          ...
          const nssf =
            Number(salary.nssf) || 0;

          const shif =
            Number(salary.shif) || 0;

          const housingLevy =
            Number(salary.housingLevy) || 0;
